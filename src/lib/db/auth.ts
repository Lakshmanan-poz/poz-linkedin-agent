...
export async function getUserForAuth(identifier: string): Promise<TeamMemberWithHash | null> {
  ...
  const cols = "id, name, email, role, auth_role, password_hash, created_at";

  // Try email first (case-insensitive), then fall back to name
  ...
  if (isEmail) {
    ...
    if (data) return data as TeamMemberWithHash;
  }
